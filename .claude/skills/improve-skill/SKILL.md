...
/improve-skill 5 weekly-reviewが完璧に動いた
/improve-skill 2 バックエンドのスキルにエラーハンドリングの説明がない
```
...
2. **影響範囲を特定する**
   ...
   - プロンプト・Claude自体の挙動に関する → `.claude/skills/prompt-guide/SKILL.md` を更新
...
- `.claude/settings.json` や `.claude/settings.local.json` は変更しない
